...
const MeetingRoom = ({ userName: propName = 'User' }) => {
  ...
  // --- Logging helper
  const log = {
    info: (...a) => console.log('[MeetingRoom]', ...a),
    warn: (...a) => console.warn('[MeetingRoom]', ...a),
    err: (...a) => console.error('[MeetingRoom]', ...a),
  };
  ...
  const setupConnection = async () => {
    ...
    pc.current.onicecandidate = (event) => {
      ...
      } else log.info('✅ ICE candidate gathering complete');
    };
    ...
    try {
      ...
      log.err('❌ Media access denied:', err);
  ...
  const sendSignal = (type, data) => {
    if (!connectedRef.current || !stompClient.current?.connected) {
      log.warn('⚠️ Tried to send signal before STOMP ready:', type);
      return;
    }
    const payload = { sender: userName, type, data };
  ...
  const handleSignal = async (signal) => {
    ...
    switch (type) {
      ...
      case 'CODE':
        ...
        break;
      default:
        log.warn('⚠️ Unknown signal:', type);
    }
  };
